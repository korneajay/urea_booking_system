...
import com.kisanurea.repositories.StockTransactionRepository;
...
public class DealerService {
    ...
    public Dealer updateStock(@NonNull String dealerId, double quantity, @NonNull String type, String farmerId) {
        ...
        if (type.equals("SOLD")) {
            ...
            checkSuspiciousActivity(dealerId);
        ...
        transaction.setFarmerId(farmerId);
    ...
    private void checkSuspiciousActivity(@NonNull String dealerId) {
        // Logic: If sales in the last hour > 100 bags, flag for review
        LocalDateTime oneHourAgo = LocalDateTime.now().minusHours(1);
        List<StockTransaction> recentSales = stockTransactionRepository.findByDealerId(dealerId)
                .stream()
                .filter(t -> t.getType().equals("SOLD") && t.getDate().isAfter(oneHourAgo))
                .toList();

        double totalSold = recentSales.stream().mapToDouble(StockTransaction::getQuantity).sum();
        if (totalSold > 100) {
            System.out.println("ALERT: Suspiciously fast stock depletion for dealer " + dealerId);
            // In a real app, set a 'isSuspicious' flag on the dealer or send an alert
        }
    }
    ...
    public void deleteDealerCompletely(@NonNull String dealerId) {
        ...
        stockTransactionRepository.deleteByDealerId(dealerId);
        java.util.Objects.requireNonNull(dealer);
